...
import br.com.tech4me.pedidos.httpClient.GaragemClient;
...
public class PedidoServiceImpl implements PedidoService{
    ...
    @Autowired
    private GaragemClient garagemClient;
    ...
    public Optional<PedidoDto> obterPedidoPorId(String id) {
        ...
        if(pedido.isPresent()){
            PedidoDto pedidoCarro = new ModelMapper().map(pedido, PedidoDto.class);
            pedidoCarro.setDadosCarro(garagemClient.obterCarro(pedidoCarro.getIdCarro()));
            return Optional.of(pedidoCarro);
        }else{
